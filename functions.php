...
function disable_enable_scripts()
{
    ...
    if (!is_admin()):
        ...
        wp_register_script('jquery', (get_template_directory_uri().'/js/custom/jquery-3.6.4.min.js'), false, '3.6.4',
            false);
    ...
    wp_enqueue_script('js-vk', get_template_directory_uri().'/js/custom/vk-superappkit.js', ['jquery'], $theme_ver, true);
    wp_enqueue_script('js-sombra', get_template_directory_uri().'/js/custom/sombra.js', ['jquery', 'js-vk'], $theme_ver, true);

    // плагин маски номера
    /*wp_enqueue_style('css-intlTelInput', get_template_directory_uri().'/styles/intlTelInput.css', [], $theme_ver);
    wp_enqueue_script('js-intlTelInput', get_template_directory_uri().'/js/custom/intlTelInput.min.js', [], $theme_ver, true);
    wp_enqueue_script('js-intlTelMak', get_template_directory_uri().'/js/custom/jquery.mask.js', [], $theme_ver, true);*/
    ...
    if (
        ...
        is_page_template('landings/landing-psy-vip.php') ||
        ...
        wp_enqueue_style('css-clo3d-media', get_template_directory_uri().'/landings/css/clo3d/media.css', [],
            $theme_ver, '(max-width:1330px)');
        wp_enqueue_script('js-clo3d-main', (get_template_directory_uri().'/landings/js/clo3d/main.js'), [], $theme_ver,
            true);
        wp_enqueue_script('js-clo3d-getreview', 'https://app.getreview.io/tags/mugUGuLoIpZqq1qD/sdk.js', [], $theme_ver,
            true);
    }
    ...
    if (is_page_template('landings/landing-kids.php')) {
        ...
        wp_enqueue_style('css-kids-media', get_template_directory_uri().'/landings/css/kids/media.css', [], $theme_ver,
            '(max-width:1200px)');
        wp_enqueue_script('js-kids-typed', (get_template_directory_uri().'/landings/js/kids/typed.umd.js'), [],
            $theme_ver, true);
        wp_enqueue_script('js-kids-main', (get_template_directory_uri().'/landings/js/kids/main.js'), [], $theme_ver,
            true);
    }
    ...
    if (
        ...
        wp_enqueue_style('css-orkt-media', get_template_directory_uri().'/landings/css/orkt/media.css', [], $theme_ver,
            '(max-width:1200px)');
    }
    ...
    if (
        ...
        wp_enqueue_style('css-difficult-site', get_template_directory_uri().'/landings/css/difficult/main.css', [],
            $theme_ver);
        wp_enqueue_style('css-difficult-media', get_template_directory_uri().'/landings/css/difficult/media.css', [],
            $theme_ver, '(max-width:1300px)');
    }
    ...
    if (is_page_template('landings/landing-beauty-hack.php') ||
        ...
        wp_enqueue_style('css-beauty-hack', get_template_directory_uri().'/landings/css/beautyhack/main.css', [],
            $theme_ver);
        wp_enqueue_style('css-beauty-hack-media', get_template_directory_uri().'/landings/css/beautyhack/media.css', [],
            $theme_ver, '(max-width:1300px)');
    }
    ...
    if (is_page_template('landings/landing-sketching.php') ||
        ...
        wp_enqueue_style('css-sketching', get_template_directory_uri().'/landings/css/sketching/main.css', [],
            $theme_ver);
        wp_enqueue_style('css-sketching-media', get_template_directory_uri().'/landings/css/sketching/media.css', [],
            $theme_ver, '(max-width:1199px)');
    }

    if (is_page_template('landings/landing-crisis-situations.php')) {
        wp_enqueue_style('css-crisis', get_template_directory_uri().'/landings/css/crisis-situations/main.css', [],
            $theme_ver);
        wp_enqueue_style('css-crisis-media', get_template_directory_uri().'/landings/css/crisis-situations/media.css',
            [], $theme_ver, '(max-width:1199px)');
    }
    ...
    if (is_page_template('landings/landing-astro-horar.php') ||
        ...
        wp_enqueue_style('css-crisis', get_template_directory_uri().'/landings/css/astro-horar/main.css', [],
            $theme_ver);
    }
    if (is_page_template('landings/landing-psy-vip.php')
    ) {
        wp_enqueue_style('css-psy-vip', get_template_directory_uri().'/landings/css/psy-vip/main.css', [],
            $theme_ver);
    }
}
...
function rest_gateway_create_lead($data)
{
    ...
    if (!is_dir($_SERVER['DOCUMENT_ROOT'] .'/wp-content/themes/talentsy/leads/'. $year)) {
        mkdir($_SERVER['DOCUMENT_ROOT'] .'/wp-content/themes/talentsy/leads/'. $year);
        mkdir($_SERVER['DOCUMENT_ROOT'] .'/wp-content/themes/talentsy/leads/'. $year, 0777, true);
    }
    
    if (!is_dir($_SERVER['DOCUMENT_ROOT'] .'/wp-content/themes/talentsy/leads/'. $year .'/'. $month)) {
        mkdir($_SERVER['DOCUMENT_ROOT'] .'/wp-content/themes/talentsy/leads/'. $year .'/'. $month);
        mkdir($_SERVER['DOCUMENT_ROOT'] .'/wp-content/themes/talentsy/leads/'. $year .'/'. $month, 0777, true);
    }
    
    if (!is_dir($_SERVER['DOCUMENT_ROOT'] .'/wp-content/themes/talentsy/leads/'. $year .'/'. $month .'/'. $day)) {
        mkdir($_SERVER['DOCUMENT_ROOT'] .'/wp-content/themes/talentsy/leads/'. $year .'/'. $month .'/'. $day);
        mkdir($_SERVER['DOCUMENT_ROOT'] .'/wp-content/themes/talentsy/leads/'. $year .'/'. $month .'/'. $day, 0777, true);
    }

    $curl = curl_init();
    curl_setopt($curl, CURLOPT_URL,
        'https://gateway.talentsy.ru/wp-content/themes/clear/inc/gateway.php?service=html-site');
    ...
    file_put_contents($_SERVER['DOCUMENT_ROOT'] .'/wp-content/themes/talentsy/leads/'. $year .'/'. $month .'/'. $day .'/'. time() .'-'.$res.'.oinfo', json_encode($data));
...
function axFormRequest()
{
    ...
    if (isset($form['special'])):
        ...
        if ($form['special'] == 'about' && $form['Theme'] == 'Другое') {
            $form['special'] = 'footer';
        }
        ...
        if ($form['special'] == 'footer'):
            ...
            if ($form['Comment']) {
                $sendData['uData']['other']['Comment'] = $form['Comment'];
            }
        ...
        if (in_array($form['special'], ['partner', 'hr', 'teacher', 'about'])):
            ...
            if ($form['rezume']) {
                $message[] = '<b>Резюме:</b>: '.$form['rezume'];
            }
            if ($form['special'] == 'hr') {
                $message[] = '<b>Желаемая вакансия:</b>: '.$form['Vacancy'];
            }
            $files = [];
            ...
            $result['mail'] = wp_mail($targetEM, $subject, implode('<br>', $message),
                ['From: Talentsy WP <wp-tech@talentsy.ru>', 'content-type: text/html'], $files);
        endif;
    else:
        $sendData['uData'] = [
            'name' => $form['Name'],
        ...
        if (isset($form['blogtaxform'])):
            $sendData['formName'] = get_field('amo_form_name',
                'category_'.$form['blogtaxform']) ?? 'Неизвестная форма в блоге';
            $sendData['utmData'] = [
                'source' => 'blog',
                'medium' => '',
                'campaign' => '',
        ...
        if (get_field('elly_alias', $pageID)) {
            $sendData['other']['ellyalias'] = get_field('elly_alias', $pageID);
        }
        if ($form['ellyalias']) {
            $sendData['other']['ellyalias'] = $form['ellyalias'];
        }
    ...
    $result['jsData'][] = 'location = "'.(get_field('redirect_form_link', $pageID) && get_field('redirect_form_link',
            $pageID) != '' ? get_field('redirect_form_link', $pageID) : get_permalink(990)).'";';
...
// подгружает автора фото из поля описания фото
...
function wrap_my_gallery_block($block_content, $block)
{
    ...
    $imageDesc = get_post_field('post_content', $imageID);

    $return = $block_content;
    if ($imageDesc):
        $return = str_replace('</figure>', '<div class="image-author">'.$imageDesc.'</div></figure>', $return);
    endif;


    $return .= '<div class="testhook" style="display:none">'.json_encode($block).'</div>';

    return $return;
}

//добавляем стили для компонента подписка
wp_enqueue_style('css-subscribe', get_template_directory_uri().'/styles/block/subscribe.css', [], '1.00');
//добавляем стили для фиксов верстки
wp_enqueue_style('css-fix', get_template_directory_uri().'/styles/fix.css', [], '1.00');
//добавляем стили для дипломов лендингов
wp_enqueue_style('css-diploma', get_template_directory_uri().'/styles/block/diploma.css', [], '1.00');